...
struct PSOut
{
    ...
    float4 normalPreview : SV_TARGET1;
};
...
PSOut PSMain(VSOut input)
    ...
    {
        PSOut outValue;
        outValue.color = texColor;
        outValue.normalPreview = float4(any(nm) ? nm.rgb : float3(0.5f, 0.5f, 1.0f), 1.0f);
        return outValue;
    }
...
float4 PSMain(VSOut input) : SV_TARGET
{
    ...
    PSOut outValue;
    outValue.color = float4(finalColor, texColor.a);
    outValue.normalPreview = float4(any(nm) ? nm.rgb : float3(0.5f, 0.5f, 1.0f), 1.0f);
    return outValue;
}