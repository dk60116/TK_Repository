...
void CSkinnedMeshRenderer::Render_Deferred(const _float3& _camPos, const _matrix& _view, const _matrix& _proj)
{
	if (!m_pMaterial)
	{
		CDebug::LogError(L"Skinned MeshRenderer - No material assigned: " + m_pGameObject->Get_ObjectNameID());
	...
	m_pMaterial->Bind_Camera_Deferred(_camPos, _view, _proj, static_cast<_uint>(m_vBones.size()));
