...
HRESULT CCamera::Initialize()
{
	...
	m_pShadowPassMat = CResources::GetInstance().LoadOnGame<CMaterial>(L"DeferredShadowMask (Material)");
	if (!m_pShadowPassMat)
	{
		CDebug::LogError("Not found DeferredShadowMask (Material)");
		return E_FAIL;
	}
	m_pShadowPassMat->AddRef();
